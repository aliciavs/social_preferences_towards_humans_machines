...
class C(BaseConstants):
    NAME_IN_URL = 'trust_human_firstmover'
...
def creating_session(subsession: Subsession):
    ...
    if subsession.round_number == 1:
        for p in subsession.get_players():
            if p.participant.id_in_session % 4 == 1:
                p.participant.role = "first1"
                p.participant.decisions = dict()
            elif p.participant.id_in_session % 4 == 3:
                p.participant.role = "first2"
                p.participant.decisions = dict()
...
class Trust(Page):
    form_model = 'player'
    form_fields = ['trust', 'belief_decision']
    @staticmethod
    def is_displayed(player: Player):
        session = player.session
        participant = player.participant
        return participant.id_in_session % 4 in [1,3]
    @staticmethod
    ...
    def before_next_page(player: Player, timeout_happened):
        participant = player.participant
        participant.decisions["trust" + str(player.round_number)] = player.trust
        participant.decisions["belief" + str(player.round_number)] = player.belief_decision
        participant.decisions["misbehaved" + str(player.round_number)] = player.misbehaved
page_sequence = [Trust]